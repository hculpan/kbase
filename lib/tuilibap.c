...
#define BASL      0x28    // holds address of screen start for current row (2 bytes)
#define RD80VID 0xC01F    // address of 40/80 column flag
#define LOWSCR  0xC054    // activate to load BASL for lower screen memory
#define HISCR   0xC055    // activate to load BASL for higher screen memory


// getscreen(x1, y1, x2, y2)
// returns a pointer to a buffer of characters in the screen area defined by corners
// x1, y1 and x2, y2
...
tgui_Menu *mptr, *nmptr;
	...
	if (win->parent ==0 ) putscreen(win->scrnchar, win->x1, win->y1, win->x2, win->y2);
	else { cleanscrn(win); }
	//cleanscrn(win);
	// menus
	mptr=win->firstmenu;
